...
UPDATE_UNIT_FILE="/etc/systemd/system/${UPDATE_SERVICE}.service"
UPDATE_PATH_FILE="/etc/systemd/system/${UPDATE_SERVICE}.path"
UPDATE_DROPIN_DIR="/etc/systemd/system/${UPDATE_SERVICE}.service.d"
UPDATE_DROPIN_FILE="${UPDATE_DROPIN_DIR}/10-install.conf"
...
[[ -f "${HERE}/${UPDATE_SERVICE}.service" ]] || die "unit file not found at ${HERE}/${UPDATE_SERVICE}.service"
[[ -f "${HERE}/${UPDATE_SERVICE}.path" ]] || die "unit file not found at ${HERE}/${UPDATE_SERVICE}.path"
[[ -f "${HERE}/app/self-update.sh" ]] || die "self-update script not found at ${HERE}/app/self-update.sh"
...
# as_bool turns an empty value into false, so the default true is applied before the conversion.
ALLOW_SELF_UPDATE="${ALLOW_SELF_UPDATE:-$(existing 'Environment=ComposeWellness__AllowSelfUpdate')}"
ALLOW_SELF_UPDATE="$(as_bool "${ALLOW_SELF_UPDATE:-true}")"
# The repository may legitimately be empty (disables the check), so an explicit empty value wins
# over the drop-in. Only an unset variable falls back.
if [[ -z "${UPDATE_REPOSITORY+x}" ]]; then
    UPDATE_REPOSITORY="$(existing 'Environment=ComposeWellness__UpdateRepository')"
    if ! grep -q '^Environment=ComposeWellness__UpdateRepository=' "${DROPIN_FILE}" 2>/dev/null; then
        UPDATE_REPOSITORY="martingertsen/compose-wellness"
    fi
fi
[[ -z "${UPDATE_REPOSITORY}" || "${UPDATE_REPOSITORY}" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]] \
    || die "UPDATE_REPOSITORY must be empty or owner/repo (got: ${UPDATE_REPOSITORY})"
...
chmod 0755 "${INSTALL_DIR}/self-update.sh"
...
echo "Installing systemd units ${UPDATE_UNIT_FILE} and ${UPDATE_PATH_FILE}"
install -m 0644 "${HERE}/${UPDATE_SERVICE}.service" "${UPDATE_UNIT_FILE}"
install -m 0644 "${HERE}/${UPDATE_SERVICE}.path" "${UPDATE_PATH_FILE}"
if [[ "${INSTALL_DIR}" != "/opt/compose-wellness" ]]; then
    mkdir -p "${UPDATE_DROPIN_DIR}"
    cat > "${UPDATE_DROPIN_FILE}" <<EOF
# Written by install.sh because INSTALL_DIR is not the default.
[Service]
Environment=INSTALL_DIR=${INSTALL_DIR}
ExecStart=
ExecStart=/bin/bash ${INSTALL_DIR}/self-update.sh
EOF
    chmod 0644 "${UPDATE_DROPIN_FILE}"
else
    rm -f "${UPDATE_DROPIN_FILE}"
fi
mkdir -p "${DROPIN_DIR}"
...
Environment=ComposeWellness__AllowSelfUpdate=${ALLOW_SELF_UPDATE}
Environment=ComposeWellness__UpdateRepository=${UPDATE_REPOSITORY}
EOF
...
systemctl enable --now "${UPDATE_PATH_FILE##*/}"
...
echo "  Self-update in UI:    ${ALLOW_SELF_UPDATE}"
echo "  Update repository:    ${UPDATE_REPOSITORY:-(disabled)}"
